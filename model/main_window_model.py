...
# Константы запросов
    ...
    RESET_DAILY_PROGRESS = """DELETE FROM habit_progress
    ...
    DELETE_HABIT_PROGRESS = """
                        DELETE FROM habit_progress WHERE habit_id = ?
                        """
    
    DELETE_HABIT_PROGRESS_MONTHLY_QUERY = """
                        DELETE FROM habits_progress_monthly WHERE habit_id = ?;
                        """
    ...
    def add_habit(self, name, category, frequency):
        ...
        self.data.execute_query_and_commit(self.ADD_HABIT_PROGRESS, progress_params)
        monthly_params = (habit_id, 0)
        self.data.execute_query_and_commit(self.ADD_MONTHLY_PROGRESS, monthly_params)
    ...
    def toggle_mark_habit(self, habit_name):
        ...
        if completed:
            self.data.execute_query_and_commit(self.UPDATE_LAST_HABIT_PROGRESS, (1, habit_id))
    ...
    def get_progress_and_target(self, habit_name):
        ...
        habit_id = self.get_habit_id(habit_name)
        params = (habit_id, )
        row = self.data.getter_for_one(self.GET_LAST_HABIT_PROGRESS_AND_TARGET, params)
        if row:
    ...
    def remove_habit(self, habit_name):
        ...
        self.data.execute_query_and_commit(self.DELETE_HABIT_PROGRESS, (habit_id,))
        self.data.execute_query_and_commit(self.DELETE_HABIT_PROGRESS_MONTHLY_QUERY, (habit_id,))
        self.data.execute_query_and_commit(self.DELETE_HABIT_QUERY, params)
    
    def is_habit_completed_today(self, habit_name):
        """Проверяет, выполнена ли привычка сегодня"""
        habit_id = self.get_habit_id(habit_name)
        params = (habit_id, )
        row = self.data.getter_for_one(self.IS_HABIT_COMPLETED_TODAY, params)
    ...
    def reset_daily_progress(self):
        ...
        self.data.execute_query_and_commit(self.RESET_DAILY_PROGRESS)
        self.init_new_progress_for_habits()

    def get_habit_id(self, habit_name):
        """Получает ID привычки по её названию"""
        params = (self.user_id, habit_name)
        row = self.data.getter_for_one(self.GET_HABIT_ID_QUERY, params)
    ...
    def today_is_new_day(self):
        ...
        params = (self.user_id,)
        row = self.data.getter_for_one(self.GET_LAST_DATE_QUERY, params)
    ...
    def init_new_progress_for_habits(self):
        ...
        habits_id_and_frequency = [(self.get_habit_id(habit["name"]), habit["daily_frequency"])  for habit in self.get_habits()]
        for habit_id, frequency in habits_id_and_frequency:
            params = (habit_id, 0, frequency)
            self.data.execute_query_and_commit(self.ADD_HABIT_PROGRESS, params)
            self.data.execute_query_and_commit(self.ADD_MONTHLY_PROGRESS, (habit_id, 0))